pipeline {
    environment {
        devImage = "ghcr.io/Theslev/joke-app:dev"
        prodImage= "ghcr.io/Theslev/joke-app:latest"
    ...
    stages {
        ...
        stage('Deploy to Development ') {
            ...
            steps {
                sh 'docker container rm -f Joke-app-dev'
                sh 'docker run --restart unless-stopped -p 5000:5000 --name Joke-app-dev -d ${devImage}'
            }
        }
        ...
        stage('Deploy to Production ') {
            when {
             ...
             steps {
                sh 'docker container rm -f Joke-app'
                sh 'docker run --restart unless-stopped -p 80:5000 --name Joke-app -d ${prodImage}'
             }
         }
    }
}
